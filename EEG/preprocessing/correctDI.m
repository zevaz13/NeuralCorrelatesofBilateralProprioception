function [DICorr] = correctDI(stimulusPresenter,DI, SR)
...
%   triggers
    ...
    for i = 1:length(inxEndH2)
        matchH2 = find(DI(inxEndH2(i):inxEndH2(i)+numSamples) == 40);
        if isempty(matchH2)
             wrongH2 = find(DI(inxEndH2(i):inxEndH2(i)+numSamples) == 60);
             indices2Correct = inxEndH2(i):inxEndH2(i)+numSamples;
             DICorr(indices2Correct(wrongH2)) = 40;
        end
    end

    for i = 1:length(inxEndMH2)
        matchMH2 = find(DI(inxEndMH2(i):inxEndMH2(i)+numSamples) == 60);
        if isempty(matchMH2)
             wrongMH2 = find(DI(inxEndMH2(i):inxEndMH2(i)+numSamples) == 40);
             indices2CorrectMH2 = inxEndMH2(i):inxEndMH2(i)+numSamples;
             DICorr(indices2CorrectMH2(wrongMH2)) = 60;
        end
    end
